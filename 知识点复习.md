...
class HeroInfo(models.Model):
    ...
    hgender = models.SmallIntegerField(choices=GENDER_CHOICES, default=0, verbose_name='性别')  # SmallIntegerField 小整数字段, 枚举
    hcomment = models.CharField(max_length=200, null=True, verbose_name='描述信息')  # null 可以为空 
...
>>> hero = HeroInfo(
    hname='孙悟空',
    hgender=0,
    hbook=book  # 外键关联, 这里是方式之一, 可以直接传递对象. 因为上面正好有查询过 book 模型对象.
)
>>> hero.save()
>>> hero2 = HeroInfo(
    hname='猪八戒',
    hgender=0,
    hbook_id=book.id  # 方式之二, 可以这几使用django自动创建的 hbook_id 字段, 然后将 关联的 book.id 赋值给它.
)
>>> hero2.save()
```
...
```python
BookInfo.objects.filter(btitle__contains='传')   # %传%
```
